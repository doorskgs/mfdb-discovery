...
def disco2consistent(mapper):
    ...
    # mapper.for_member('names', lambda opt: set(json.loads(opt)))
    # todo: handle description later
    #mapper.for_member('description', )

    mapper.for_member('edb_id', lambda opt: force_flatten(opt.edb_id, opt.attr_mul))
    mapper.for_member('edb_source', lambda opt: force_flatten(opt.edb_source, opt.attr_mul))
